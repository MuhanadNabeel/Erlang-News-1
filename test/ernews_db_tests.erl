...
db_write_test()->
    ...
    ?assertEqual({ok,updated},
		 ernews_db:query_function(write,"Delete from ernews_broken where Url = 'test'")), 
...
%--------------------------------------------------------------------------------%
db_duplicate_test()->
    ?assertEqual(false,ernews_db:query_function(exists,
"select count(a.Url) from ernews_news a, ernews_news b where a.Url = b.Url AND a.newsID != b.newsID")).
